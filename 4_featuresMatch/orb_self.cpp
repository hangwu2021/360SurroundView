...
int main(int argc, char *argv[])
{
    if (argc != 3)
    {
        std::cerr << "usage: ./orb_self 1.png 2.png" << std::endl;
        exit(EXIT_FAILURE);
    }
    ...
    // Step1: Detect Corners
    std::chrono::steady_clock::time_point t1 = std::chrono::steady_clock::now();
    
    std::vector<cv::KeyPoint> points1;
    cv::FAST(img1, points1, 40);
    std::vector<DescType> descriptors1;
    ComputeORB(img1, points1, descriptors1);
    
    std::vector<cv::KeyPoint> points2;
    cv::FAST(img2, points2, 40);
    std::vector<DescType> descriptors2;
    ComputeORB(img2, points2, descriptors2);
    ...
    cv::Mat img_show;
    cv::drawMatches(img1, points1, img2, points2, matches, img_show);
    cv::imwrite("img_show.png", img_show);
